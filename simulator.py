...
class Run(QThread):
    ...
    def __init__(self, system, dataset):
        ...
        for key, value in self.dataset.items():
            if type(value) is list:
                self.dataset[key] = [tuple(map(float, v)) for v in value]
            elif type(value) is tuple:
                self.dataset[key] = tuple(map(float, value))
            else:
                self.dataset[key] = float(value)
        self.log = {
            'x': [],
            'y': [],
            'front dist': [],
    ...
    def run(self):
        ...
        self.sig_paint_init.emit(self.car.get_pos_angle(), self.dataset['walls'],
                                 (self.dataset['finishline_l'], self.dataset['finishline_r']))
        while True:
            if (finish_line_x[0] < self.car.pos[0] < finish_line_x[1]) and (
                    finish_line_y[0] < self.car.pos[1] < finish_line_y[1]):
                self.exit = True
